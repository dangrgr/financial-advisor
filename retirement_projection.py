...
def print_savings_table(p, ret_dan_age=55):
    ...
    print(f"  Terri's own IRA (non-deductible):      {fmt(totals['_nondeduct_ira'])}"
          f"   — her account; basis tax-free, growth taxed as income")
    print(f"  {'-'*70}")
    print(f"  TOTAL saved over {len(rows)} working years:    {fmt(totals['_total'])}")
    print()
    print("NOTES / caveats for the year-over-year conversation:")
    print("  • Terri's IRA is tracked as its own vessel because it's a REAL account she")
    print("    funds for ownership, not for tax efficiency. The model handles it honestly:")
    print("    basis comes back tax-free and only growth is taxed, at the ordinary rate —")
    print("    so it lands mildly WORSE than taxable, which is correct. The deduction /")
    print("    pro-rata fine print is a CPA detail that doesn't change this allocation view;")
    print("    her clean pre-tax route, if she ever wants one, is a SOLO-401(k) off")
    print("    consulting income.")
